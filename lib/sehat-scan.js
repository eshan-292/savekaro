...
function parseReportText(text) {
  ...
  // More specific patterns — name must start with a letter, value must be a reasonable number
  const patterns = [
    // "Hemoglobin : 14.5 g/dL 13.0 - 17.5" or "Hemoglobin:14.5"
    /^([A-Za-z][A-Za-z0-9\s\(\)\/%\.\-]{1,50}?)\s*[:=]\s*(\d+\.?\d*)\s*(.*?)$/,
    // "Hemoglobin  14.5  g/dL  13.0 - 17.5" (spaced format — require 2+ spaces before number)
    /^([A-Za-z][A-Za-z0-9\s\(\)\/%\.\-]{2,50}?)\s{2,}(\d+\.?\d*)\s+(.*?)$/,
    // "Hemoglobin (Hb) 14.5 g/dL" or "HbA1c 5.8 %"
  ...
  for (const line of lines) {
    ...
    for (const pattern of patterns) {
      ...
      // Skip if "name" is just numbers, too short, or a skip word
      if (/^\d+\.?\d*$/.test(rawName)) continue;
      if (rawName.length < 2) continue;

      // Clean rawName — remove trailing dots, colons, dashes
      rawName = rawName.replace(/[\s:;\-–—\.,]+$/, '').trim();
      if (rawName.length < 2) continue;

      // Check if rawName is a skip word after cleaning
      const rawLower = rawName.toLowerCase().trim();
      const words = rawLower.split(/\s+/);
      // Skip if the ENTIRE rawName is a single skip word (but allow compound names like "Total Bilirubin")
      if (words.length === 1 && SKIP_WORDS.has(rawLower)) continue;
      // Skip if ALL words are skip words (e.g., "total amount" but NOT "total bilirubin")
      if (words.length > 1 && words.every(w => SKIP_WORDS.has(w))) continue;

      const param = findParameter(rawName);
      if (!param || seen.has(param.id)) continue;

      // Strict plausibility check — value must be in a reasonable range
      const ranges = param.ranges.default || param.ranges.male || param.ranges.female;
      if (ranges) {
        const plausibleMax = ranges.high * 3; // Tightened from 5x to 3x
        const plausibleMin = ranges.low > 0 ? ranges.low * 0.05 : 0; // Tightened
        if (value > plausibleMax) continue;
        if (ranges.low > 0 && value < plausibleMin && value !== 0) continue;
        // Reject values that look like years, phone numbers, dates, etc.
        if (value > 1000 && ranges.high < 500) continue;
        if (value > 10000 && ranges.high < 5000) continue;
      }
