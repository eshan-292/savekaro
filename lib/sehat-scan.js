...
const CLINICAL_PATTERNS = [
  {
    ...
    suppress: ['hemoglobin', 'mcv', 'mch', 'mchc', 'iron', 'iron_saturation', 'ferritin', 'tibc', 'rdw', 'rdw_sd', 'hematocrit']
  },
  {
    id: 'b12_deficiency',
...
function analyzeResults(parsedResults, gender = 'default') {
  ...
  for (const result of parsedResults) {
    ...
    const ranges = (result.reportRange && result.reportRange.low !== undefined && result.reportRange.high !== undefined)
      ? result.reportRange
      : dbRanges;
    const rangeSource = (result.reportRange && result.reportRange.low !== undefined) ? 'report' : 'database';
  ...
  if (critical.length > 0) {
    analysis.summary = `${critical.length} parameter(s) are significantly outside normal range. A doctor consultation is recommended to discuss these findings.`;
  } else if (abnormal.length > 3) {
    analysis.summary = `${abnormal.length} parameters are outside normal range. Consider discussing these with your doctor at your next visit.`;
  } else if (abnormal.length > 0) {
    analysis.summary = `Most parameters are normal. ${abnormal.length} parameter(s) are slightly outside range — usually manageable with diet and lifestyle changes.`;
  } else {
  ...
  // Step 2: Add doctor consultations
  const doctorsNeeded = [...new Set(abnormal.map(p => p.doctor).filter(Boolean))];
  if (doctorsNeeded.length > 0) {
    analysis.recommendations.unshift(`🩺 Consult: ${doctorsNeeded.join(', ')}`);
  }

  // Step 3: Add individual diet tips for abnormal params NOT covered by patterns
  for (const p of abnormal) {
    ...
    if (dietAdvice && !analysis.recommendations.includes(dietAdvice)) {
