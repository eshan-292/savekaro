...
function analyzeResults(parsedResults, gender = 'default') {
  ...
  // Generate recommendations — use the RIGHT diet based on whether parameter is high or low
  ...
  for (const p of abnormal) {
    ...
    if (hasHighLow) {
      // Split on "High X:" and "Low X:" patterns and pick the right part
      const highMatch = dietAdvice.match(/high\s+\w+\s*:\s*(.*?)(?=\.\s*low\s+\w+\s*:|$)/is);
      const lowMatch = dietAdvice.match(/low\s+\w+\s*:\s*(.*?)(?=\.\s*high\s+\w+\s*:|$)/is);

      if (isLow && lowMatch) {
        dietAdvice = lowMatch[1].trim();
      } else if (isHigh && highMatch) {
        dietAdvice = highMatch[1].trim();
      }
    }

    if (dietAdvice && !diets.includes(dietAdvice)) {
      diets.push(dietAdvice);
    }
  }
