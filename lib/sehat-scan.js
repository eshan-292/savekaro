...
const PARAMETERS_DB = [
  ...
  },
  {
    id: 'urea',
    name: 'Blood Urea',
    aliases: ['urea', 'blood urea', 'bun', 'blood urea nitrogen', 'serum urea', 'urea nitrogen'],
    unit: 'mg/dL',
    category: 'Kidney Function',
    ranges: { default: { low: 7, high: 20 } },
    highMeaning: 'High urea — kidney not filtering properly. Dehydration bhi ho sakta hai. Paani zyada piyein.',
    lowMeaning: 'Low urea — liver disease or very low protein diet.',
    doctor: 'Nephrologist',
  ...
  },
  {
    ...
    aliases: ['t3', 'triiodothyronine', 'total t3', 'serum t3', 'ft3', 'free t3'],
    unit: 'ng/dL',
    category: 'Thyroid',
    ranges: { default: { low: 80, high: 200 } },
  ...
  },
  {
    id: 't4',
    name: 'T4 (Thyroxine)',
    aliases: ['t4', 'thyroxine', 'total t4', 'serum t4', 'ft4', 'free t4'],
    unit: 'µg/dL',
    category: 'Thyroid',
...
function findParameter(rawName) {
  ...
  // Contains match — but require minimum 3 chars to avoid noise
  for (const param of PARAMETERS_DB) {
    for (const alias of param.aliases) {
      const normAlias = normalizeParamName(alias);
      if (normAlias.length < 3) {
        // Short aliases (hb, rbc, etc.) need exact match only
        if (normalized === normAlias) return param;
        continue;
      }
      if (normalized.includes(normAlias) || normAlias.includes(normalized)) {
        if (Math.min(normalized.length, normAlias.length) >= 3) return param;
      }
    }
  }

  // Fuzzy match with Levenshtein — tighter threshold for OCR
  let bestMatch = null;
  let bestScore = Infinity;
  for (const param of PARAMETERS_DB) {
    for (const alias of param.aliases) {
      const normAlias = normalizeParamName(alias);
      if (normAlias.length < 4) continue; // Skip short aliases for fuzzy
      const dist = levenshtein(normalized, normAlias);
      // Tighter threshold: max 20% of alias length, min 1
      const threshold = Math.max(1, Math.floor(normAlias.length * 0.2));
...
function parseReportText(text) {
  ...
  // Multiple patterns to handle different lab report formats and OCR noise:
  const patterns = [
    // "Hemoglobin  14.5  g/dL  13.0 - 17.5"
    /^(.+?)[\s:;\-–—]+(\d+\.?\d*)\s*(.*?)$/,
    // "Hemoglobin : 14.5" or "Hemoglobin:14.5"
    /^(.+?)\s*[:=]\s*(\d+\.?\d*)\s*(.*?)$/,
    // "14.5  Hemoglobin" (value before name — some reports)
    /^(\d+\.?\d*)\s{2,}(.+?)$/,
  ...
  for (const line of lines) {
    ...
    if (/^(date|time|name|patient|dr\.|doctor|lab|report|page|printed|specimen|sample|collected|ref\s|reference)/i.test(trimmed)) continue;
    if (/^\d+\/\d+\/\d+/.test(trimmed)) continue; // Date lines
    ...
    for (const pattern of patterns) {
      ...
      // Check if this is the "value first" pattern
      ...
      // Sanity check: is the value in a plausible range for this parameter?
      // Reject wildly implausible values (e.g., Hemoglobin = 2024)
      const ranges = param.ranges.default || param.ranges.male || param.ranges.female;
      if (ranges) {
        const plausibleMax = ranges.high * 5;
        const plausibleMin = ranges.low * 0.01;
        if (value > plausibleMax || (value < plausibleMin && value !== 0)) continue;
      }

      seen.add(param.id);

      // Try to extract range from the rest of the line
      let reportRange = null;
      const rest = match[3] || match[match.length - 1] || '';
      const rangeMatch = rest.match(rangePattern);
      if (rangeMatch) {
        reportRange = { low: parseFloat(rangeMatch[1]), high: parseFloat(rangeMatch[2]) };
      }
...
function analyzeResults(parsedResults, gender = 'default') {
  ...
  for (const result of parsedResults) {
    ...
    const ranges = param.ranges[gender] || param.ranges.default;
    let status, statusEmoji, paramScore, explanation;

    if (result.value < ranges.low) {
      const deviation = ((ranges.low - result.value) / ranges.low) * 100;
      if (deviation > 30) {
        status = 'critical_low';
        statusEmoji = '🔴';
        paramScore = 20;
        explanation = param.lowMeaning;
        analysis.criticalFlags.push(`${param.name} is critically low at ${result.value} ${param.unit}`);
      ...
      const deviation = ((result.value - ranges.high) / ranges.high) * 100;
      if (deviation > 30) {
        status = 'critical_high';
        statusEmoji = '🔴';
        paramScore = 20;
        explanation = param.highMeaning;
        analysis.criticalFlags.push(`${param.name} is critically high at ${result.value} ${param.unit}`);
      } else if (deviation > 15) {
        status = 'high';
        statusEmoji = '🟡';
        paramScore = 55;
