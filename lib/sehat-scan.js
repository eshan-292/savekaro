...
const PARAMETERS_DB = [
  ...
  // ─── DIABETES / BLOOD SUGAR ───
  {
    ...
    aliases: ['fasting blood sugar', 'fbs', 'fasting glucose', 'fasting blood glucose', 'fbg', 'glucose fasting', 'blood sugar fasting', 'fasting plasma glucose', 'fpg'],
    unit: 'mg/dL',
    category: 'Diabetes',
...
function findParameter(rawName) {
  ...
  for (const param of PARAMETERS_DB) {
    for (const alias of param.aliases) {
      ...
      if (normalized.includes(normAlias) || normAlias.includes(normalized)) {
        if (Math.min(normalized.length, normAlias.length) >= 2) return param;
      }
    }
  }
  ...
  for (const param of PARAMETERS_DB) {
    for (const alias of param.aliases) {
      ...
      const threshold = Math.max(1, Math.floor(normAlias.length * 0.3));
...
function parseReportText(text) {
  ...
  // Common patterns for lab report lines:
  // "Hemoglobin  14.5  g/dL  13.0 - 17.5"
  // "Hemoglobin : 14.5 g/dL"
  // "HB - 14.5"

  const valuePattern = /^(.+?)[\s:;\-–—]+(\d+\.?\d*)\s*(.*?)$/;
