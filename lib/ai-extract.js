...
- eGFR / Estimated GFR (often shown as ">90" or ">60" — extract as value: 90 or 120 if shown as ">90", with refRange: "90 - 200")
...
IMPORTANT: If a value is shown as ">90" or ">120", extract the number (90 or 120) as the value. Do NOT skip parameters just because they show ">" prefix.
...
const UNIT_CONVERSIONS = {
  ...
  // WBC Total: lab may report in ×10³/µL (4.49), our DB uses cells/µL (4000-11000)
  'wbc': {
    detect: (reportUnit, value) => {
      const u = reportUnit.toLowerCase().replace(/\s/g, '');
      // If unit contains 10³/10^3/×10 AND value is small (< 20), it's in thousands
      // Also detect by value alone: if WBC value < 30 and no clear cells/µL unit, it's likely in thousands
      const isThousandsUnit = u.includes('10³') || u.includes('10^3') || u.includes('x10') || u.includes('×10') || u.includes('10*3') || u.includes('thou');
      return isThousandsUnit && value < 100;
    },
    convert: (value) => Math.round(value * 1000), // 4.49 ×10³ = 4490 cells/µL
    convertRange: (low, high) => ({ low: Math.round(low * 1000), high: Math.round(high * 1000) }),
  },
...
function validateReportRange(reportRange, dbRanges, value, paramName) {
  ...
  // the AI likely picked the wrong tier — UNLESS the ranges meaningfully overlap
  // (which means the lab just has a narrower/different reference range)
  const valueInDb = value >= dbLow && value <= dbHigh;
  const valueOutOfReport = value < rLow || value > rHigh;
  const rangesOverlap = rLow < dbHigh && rHigh > dbLow; // True if ranges share any interval

  if (valueInDb && valueOutOfReport) {
    if (rangesOverlap) {
      // Lab range overlaps with DB range — it's a legitimate clinical variation (narrower lab range)
      // Trust the lab's more specific range
      console.log(`  ✓ Range validation: ${paramName} lab range (${rLow}-${rHigh}) overlaps DB (${dbLow}-${dbHigh}). Trusting lab range.`);
    } else {
      // Ranges don't overlap at all — AI definitely picked wrong tier
...
function mapToParametersDB(aiResults, PARAMETERS_DB, findParameter, gender) {
  ...
  for (const item of aiResults) {
    ...
    // Safety net: auto-detect scale mismatch even if UNIT_CONVERSIONS didn't fire
    // e.g., WBC value 4.49 vs DB range 4000-11000 → clearly in thousands, multiply by 1000
    const dbMid = (dbRanges.low + dbRanges.high) / 2;
    if (dbMid > 0 && value > 0) {
      const valueToDbRatio = dbMid / value;
      // If DB values are ~1000x bigger, the value is likely in thousands
      if (valueToDbRatio > 500 && valueToDbRatio < 2000) {
        console.log(`  ⚠ Scale safety net: ${param.name} value ${value} is ~1000x smaller than DB mid ${dbMid}. Auto-converting ×1000.`);
        value = Math.round(value * 1000);
        if (reportRange) {
          reportRange = { low: Math.round(reportRange.low * 1000), high: Math.round(reportRange.high * 1000) };
        }
        unit = param.unit;
      }
      // If DB values are ~100x bigger (e.g., platelets in lakh)
      else if (valueToDbRatio > 50 && valueToDbRatio < 200) {
        console.log(`  ⚠ Scale safety net: ${param.name} value ${value} is ~100x smaller than DB mid ${dbMid}. Auto-converting ×100.`);
        value = Math.round(value * 100);
        if (reportRange) {
          reportRange = { low: Math.round(reportRange.low * 100), high: Math.round(reportRange.high * 100) };
        }
        unit = param.unit;
      }
    }
...
async function generateDoctorSummary(analysis, gender, age) {
  ...
  const abnormalParams = analysis.parameters.filter(p => p.status !== 'normal' && !p.status.startsWith('borderline'));
  const borderlineParams = analysis.parameters.filter(p => p.status.startsWith('borderline'));
  const normalParams = analysis.parameters.filter(p => p.status === 'normal');

  // Build parameter listing for the prompt — group by severity for clarity
  const criticalParams = analysis.parameters.filter(p => p.status.startsWith('critical'));
  const mildAbnormal = abnormalParams.filter(p => !p.status.startsWith('critical'));
  ...
  const abnormalListing = abnormalParams.map(p => {
    const severity = p.status.startsWith('critical') ? '⚠️ CRITICAL' : p.status.includes('slightly') ? 'MILDLY' : 'MODERATELY';
    return `${severity} ${p.status.includes('low') ? 'LOW' : 'HIGH'}: ${p.name}: ${p.value} ${p.unit} (normal range: ${p.normalRange})`;
  }).join('\n');

  const borderlineListing = borderlineParams.map(p => {
    return `BORDERLINE: ${p.name}: ${p.value} ${p.unit} (normal range: ${p.normalRange}, within range but near boundary)`;
  }).join('\n');

  // Detected patterns for context
  const patternContext = analysis.detectedPatterns && analysis.detectedPatterns.length > 0
    ? `\nDetected Clinical Patterns: ${analysis.detectedPatterns.join(', ')}`
    : '';

  const genderLabel = gender === 'female' ? 'Female' : gender === 'male' ? 'Male' : 'Patient';

  const summaryPrompt = `You are an experienced, careful Indian doctor (MBBS MD) doing a warm consultation explaining blood test results to a patient in Hinglish (mix of Hindi and English).

Patient: ${genderLabel}, age ${age}

Blood Test Results (${analysis.parameters.length} parameters tested):
${paramListing}

ABNORMAL Findings (${abnormalParams.length} parameters):
${abnormalListing || 'None'}

BORDERLINE Findings (${borderlineParams.length} — within range but near boundary):
${borderlineListing || 'None'}
${patternContext}

YOUR TASK: Write a detailed, warm clinical summary in 4-5 short paragraphs. This is the most important part of the report — the patient relies on this to understand their health.

MANDATORY STRUCTURE:
1. REASSURANCE: Start with what's good — how many parameters are normal, mention specific normal categories
2. KEY FINDINGS: Explain EACH abnormal finding by name and what it means clinically. Be specific — don't just say "thoda zyada hai", explain the clinical significance. If iron/hemoglobin are low, explicitly say "iron deficiency anemia ka pattern dikh raha hai"
3. CONNECTIONS: Connect related findings into clinical patterns (e.g., low Hb + low iron + low iron saturation + high TIBC = iron deficiency anemia; high FBS = pre-diabetes risk). This is critical — patients need to understand how findings relate
4. DIET/LIFESTYLE: Give 3-4 SPECIFIC diet tips using Indian foods, relevant ONLY to the abnormal findings. Be specific: "palak, chukandar, anaar, khajoor khayein" not just "healthy food khayein"
5. NEXT STEPS: Recommend which doctor to see and what follow-up tests, in a gentle non-alarming tone
...
const PARAMETER_GROUPS = {
  'cbc_core': {
    trigger: ['hemoglobin', 'rbc', 'wbc', 'platelets'],        // if any CBC param found
    expect: ['hemoglobin', 'rbc', 'wbc', 'platelets', 'hematocrit', 'mcv', 'mch', 'mchc', 'rdw', 'mpv', 'esr'],
    names: ['Hemoglobin', 'RBC Count', 'WBC / Total Leucocyte Count', 'Platelet Count', 'Hematocrit / PCV', 'MCV', 'MCH', 'MCHC', 'RDW-CV', 'MPV', 'ESR']
  },
  'wbc_differential_pct': {
    trigger: ['hemoglobin', 'wbc'],                              // if CBC found, expect WBC differential
    expect: ['neutrophils', 'lymphocytes', 'monocytes', 'eosinophils', 'basophils'],
    names: ['Neutrophils %', 'Lymphocytes %', 'Monocytes %', 'Eosinophils %', 'Basophils %']
  },
  'wbc_absolute': {
  ...
  'iron_studies': {
    trigger: ['iron', 'tibc', 'ferritin', 'iron_saturation'],
    expect: ['iron', 'tibc', 'uibc', 'ferritin', 'iron_saturation'],
    names: ['Serum Iron', 'TIBC', 'UIBC', 'Ferritin', 'Iron Saturation / Transferrin Saturation']
  },
  'kidney': {
    trigger: ['bun', 'creatinine', 'egfr', 'urea'],
    expect: ['urea', 'bun', 'creatinine', 'egfr', 'uric_acid'],
    names: ['Serum Urea / Blood Urea', 'BUN (Blood Urea Nitrogen)', 'Creatinine', 'eGFR', 'Uric Acid']
  },
  'liver': {
    trigger: ['sgot', 'sgpt', 'bilirubin_total'],
    expect: ['sgot', 'sgpt', 'alp', 'ggt', 'bilirubin_total', 'bilirubin_direct', 'bilirubin_indirect', 'total_protein', 'albumin', 'globulin', 'ag_ratio'],
    names: ['SGOT/AST', 'SGPT/ALT', 'ALP (Alkaline Phosphatase)', 'GGT', 'Bilirubin Total', 'Bilirubin Direct', 'Bilirubin Indirect', 'Total Protein', 'Albumin', 'Globulin', 'A/G Ratio']
  },
  'lipid': {
    trigger: ['total_cholesterol', 'ldl', 'hdl'],
    expect: ['total_cholesterol', 'ldl', 'hdl', 'triglycerides', 'vldl', 'non_hdl_cholesterol', 'ldl_hdl_ratio'],
    names: ['Total Cholesterol', 'LDL Cholesterol', 'HDL Cholesterol', 'Triglycerides', 'VLDL', 'Non-HDL Cholesterol', 'LDL/HDL Ratio']
  },
  'thyroid': {
    trigger: ['tsh', 't4', 't3', 'ft3', 'ft4'],
    expect: ['tsh', 't3', 't4'],
    names: ['TSH', 'Total T3 / Triiodothyronine', 'Total T4 / Thyroxine']
  },
  'diabetes': {
    trigger: ['fasting_glucose', 'hba1c', 'pp_glucose'],
    expect: ['fasting_glucose', 'hba1c'],
    names: ['Fasting Blood Sugar / Glucose', 'HbA1c (Glycated Hemoglobin)']
  },
  'vitamins': {
    trigger: ['vitamin_d', 'vitamin_b12', 'calcium'],
    expect: ['vitamin_d', 'vitamin_b12', 'calcium'],
    names: ['Vitamin D (25-Hydroxy)', 'Vitamin B12', 'Calcium']
  },
  'electrolytes': {
    trigger: ['sodium', 'potassium', 'chloride'],
    expect: ['sodium', 'potassium', 'chloride'],
    names: ['Sodium', 'Potassium', 'Chloride']
  }
};
